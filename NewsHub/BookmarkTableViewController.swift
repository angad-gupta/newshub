...
class BookmarkTableViewController: UITableViewController {
    ...
    {
        print("loading data")
        if let articlesJson = UserDefaults.standard.data(forKey: "bookmarks") {
            do {
                // Create JSON Decoder
                let decoder = JSONDecoder()

                // Decode Note
                let articles = try decoder.decode([Article].self, from: articlesJson)
                print(articles)
...
//                })
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "bookmarktableview", for: indexPath) as! BookmarkTableViewCell
        
        let rowNumber = indexPath.row
        let thisRowData = articles[rowNumber]
        
        cell.btitle.text = thisRowData.title
        cell.bdesc.text = thisRowData.description
        cell.vc = self;
        cell.index = rowNumber;
        cell._url = thisRowData.url!
        cell.article = articles[rowNumber]
        
        let url = URL(string: ((thisRowData.urlToImage == nil || thisRowData.urlToImage!.isEmpty ) ? "https://kubalubra.is/wp-content/uploads/2017/11/default-thumbnail.jpg" : thisRowData.urlToImage)!)
